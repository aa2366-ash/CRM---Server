...
app.get('/dashboard' ,[authorize] ,async (req,res) => {
    ...
    try {
        ...
        var [service, leads, contacts] = await  Promise.all([db.collection("service").find().toArray() ,
...
app.put('/service' , [authorize] , async (req, res) => {
    try {
        ...
        var data = await db.collection("users").findOne({email : res.locals.user.email })
        if(data !== null ){
            if(data.permission !== "edit"){
                ...
                var contactmatch = await db.collection("contacts").findOne({email : reqData.email })
                ...
                var service = await db.collection("service").updateOne( {_id : mongodb.ObjectID(reqData._id)} , {$set : {status : reqData.status , description : reqData.description , email : reqData.email , lastEditedAt : new Date() } } )
    ...
    try {
        ...
        if(data !== null ){
            if(data.permission !== "edit"){
                ...
                reqData.createdBy = {name : data.name , email : data.email , access : data.access }
    ...
    try {
        ...
        if(data !== null ){
            if(data.permission !== "edit"){
                ...
                var leads = await db.collection("leads").updateOne( {_id : mongodb.ObjectID(reqData._id)} , {$set : {status : reqData.status , description : reqData.description , email : reqData.email , lastEditedAt : new Date() } } )
...
app.post('/access' , [authorize] , async (req,res) => { 
    ...
    try {
        // do something .... 
        const client = await mongodb.connect(process.env.DBURL, {useNewUrlParser: true , useUnifiedTopology: true } );
        const db = client.db("crm");
        var data = await db.collection("users").findOne({email : res.locals.user.email })
        if(data.access === 'manager' || data.access === 'admin' && data.permission === "edit"  ) {
           // res.json({message : 'dashboard' , user : res.locals.user })
           var users = await db.collection("users").updateOne({_id : mongodb.ObjectID(req.body._id) } , {$set : {permission : req.body.permission} })
...
app.get('/access' , [authorize] , async (req,res) => { 
    ...
    try {
        ...
        if( ( data.access === 'manager' || data.access === 'admin' ) && ( data.permission === 'view' || data.permission === "edit" ) ) {
           // res.json({message : 'dashboard' , user : res.locals.user })
           var users = await db.collection("users").find({access : {$not : /^admin/ } } , {password : 0 } ).toArray()
...
app.post('/login', async (req,res) => {
    ...
    try {
        ...
        if(result){
            ...
            let jwtToken = jwt.sign({user : data} , process.env.JWTTK , {expiresIn : '1h'})
